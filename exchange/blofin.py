...
class BlofinScraper(BaseScraper):
    def __init__(self, analyzer: DeepSeekAnalyzer, debug: bool = False, max_size: int = 10, offset_days: int = 7, analyzer_api_key= None):
        super().__init__("blofin", "https://www.blofin.com", analyzer, debug, max_size, offset_days, analyzer_api_key)
    ...
    async def get_announcements_id(self, catalog_id='161', page_no='1', page_size='10'):
        ...
        if len(announcement_url) == 0:
            self.log("ERROR", "未找到blofin公告链接")
            return
        
        self.log("INFO", f"blofin公告链接: {announcement_url}")
        ...
        self.log("INFO", f"section_tags: {len(section_tags)}")
    ...
    async def get_announcement_detail(self, full_url):
        ...
        try:
            ...
            if article_body:
                ...
                self.log("INFO", "成功找到article-body标签")
            else:
            ...
            try:
                ...
                if article_author:
                    ...
                    if time_element:
                        ...
                        self.log("INFO", f"release_time: {release_time}")
                    else:
                        self.log("ERROR", "未找到time元素")
                        release_time = None
                else:
                    self.log("ERROR", "未找到article-author元素")
                    release_time = None
    ...
    async def run_scraping(self):
        ...
        try:
            ...
            for i, article in enumerate(announcements):
                ...
                # text_file_name = os.path.join(self.output_dir, f"blofin_{article_id}.txt")
                json_file_name = os.path.join(self.output_dir, f"blofin_{article_id}.json")
                ...
                if full_url:
                    ...
                    if release_time_str < (pd.Timestamp.now(tz='Asia/Hong_Kong') - pd.Timedelta(days=self.offset_days)).strftime('%Y-%m-%d %H:%M:%S'):
                        ...
                        with open(json_file_name, 'w', encoding='utf-8') as f:
                            json.dump({'release_time': release_time_str, 'text': "", 'url': full_url, 'title': article.get('title', 'N/A'),"exchange": "blofin"}, f, ensure_ascii=False, indent=4)
                        continue
                    if detail_result:
                        self.log("INFO", "纯文字内容:")
...
async def main():
    """主函数，创建爬虫实例并运行"""
    analyzer = DeepSeekAnalyzer(api_key="sk-790c031d07224ee9a905c970cefffcba")
    scraper = BlofinScraper(analyzer, debug=True, max_size=3)
