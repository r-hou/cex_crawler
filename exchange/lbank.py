...
class LbankScraper(BaseScraper):
    ...
    def get_session_id(self):
        ...
        self.log("DEBUG", f"找到 {len(all_links)} 个链接")
        for link in all_links:
            ...
            for target_text in target_texts:
                if target_text in link_text.lower():
                    self.log("DEBUG", f"检查链接: {link_text}")
                    target_links.append({
                        'text': link_text,
                        'href': href,
                        'full_url': f"https://www.lbank.com{href}" if href and href.startswith('/') else href
                    })
                    self.log("INFO", f"找到目标链接: {link_text} -> {href}")
                    break  # 避免重复添加同一个链接
        self.log("INFO", f"目标链接列表: {target_links}")
        # 如果没有找到，尝试其他方法
        if not target_links:
            self.log("WARNING", "未找到目标链接，尝试其他方法...")
            ...
            for link in class_links:
                ...
                self.log("INFO", f"通过class找到链接: {link_text} -> {href}")
            ...
            for link in id_links:
                ...
                self.log("INFO", f"通过id找到链接: {link_text} -> {href}")
            ...
            for link in data_links:
                ...
                self.log("INFO", f"通过data属性找到链接: {link_text} -> {href}")
        ...
        try:
            ...
            if next_data_script and next_data_script.string:
                try:
                    ...
                    self.log("ERROR", f"解析 __NEXT_DATA__ JSON失败: {traceback.format_exc()}", console=True)
            
            
            if not json_data:
                self.log("ERROR", "未找到script标签中的JSON数据", console=True)
            
            return json_data
            
        except Exception as e:
            self.log("ERROR", f"提取script标签JSON数据失败: {traceback.format_exc()}", console=True)
            return {}
    ...
    async def run_scraping(self):
        ...
        for i, announcement in enumerate(announcements):
            ...
            self.log("INFO", f"   URL: https://www.lbank.com/support/articles/{announcement.get('code', '')}")
